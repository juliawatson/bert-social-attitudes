...
def compute_log_likelihoods(morph_type, output_format_str):
    ...
    data = data.drop(columns=["gender"])  # drop because it's in both
    pre_join_data_len = len(data)
    data = data.join(bert_data.set_index(["name", "lexeme"]), on=("name", "lexeme"))
    assert len(data) == pre_join_data_len
        ...
        with open(output_path, "w") as f:
            csv_writer = csv.DictWriter(
                f, fieldnames=["group", "loglik for men's names",
                            ...
                            "n_participants", "n_observations_mens_names", "n_observations_womens_names",
            ...
            for poli_party in ["Democrat", "Non-Partisan", "Republican"]:
                ...
                csv_writer.writerow({
                    ...
                    "n_participants": len(set(curr_data["workerid"])),
                    "n_observations_mens_names": len(curr_men_names),
                    "n_observations_womens_names": len(curr_women_names),
                    "n_observations_total": len(curr_data)
                })
